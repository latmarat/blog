---
layout: post
title: "Stress-strain data from ABAQUS simulation using Python"
description: "The post describes a Python script for obtaining stress--strain data from ABAQUS simulations with a representative volume elements of heterogeneous microstructure."
modified: 2015-05-12
tags: [abaqus, python]
mathjax: true
long: true
...
The post describes a Python script for obtaining stress--strain data from ABAQUS simulations with a representative volume elements of heterogeneous microstructure
...
where `fname` is the file name of the output database (without extension), `instanceName` is the instance name of your part of interest (e.g. _'PART-1-1'_), and `setName` is the name of the element set that contains the region for which calculations shall be done.  
...
The script writes some runtime messages to a file `odb_aba-ss.txt` created in the folder where `aba-ss.py` was run. In this file, you can see some useful information, such as, for which Step, instance, element set, the calculations are carried out as well as how many time frames  are available in your odb.
...
The function is defined with _'ALLELEMENTS'_ as a default name for the element set name for which calculation is performed. Also, the first instance available in the odb is chosen as default. For example, if I have an odb with a single instance _PART-1-1_, the following two calls of the function will yield  exactly the same results
